...
additionally verified by live browser automation. The Word Book entry is
**new** as of 2026-09-17, covering a long iterative ribbon/menu/sharing/print
fix pass plus a new Document tabs feature (a `.book` format v2 change); unlike
the three entries above, it is **not** verified by live browser automation —
only by typecheck and dev-server module resolution — and says so in its own
status line.

**Responsive layout is a separate axis (2026-09-21).** Fourteen applications were
made usable on phones and tablets in a one-app-at-a-time pass, and each of those
entries carries a **Responsive** bullet. That work was verified live in a real
browser (Playwright) for *layout and interaction at emulated viewport widths* —
which does **not** upgrade any entry's own status line: an app whose status says
"UI unverified" is still unverified for everything except its narrow-viewport
layout. Spreadsheet, Presentation, Browser, Terminal and the Launcher app were not
part of the pass. Full behaviour, method and gaps are in
...
* **Responsive, added 2026-09-21** Below 1024px the sidebar becomes a menu-triggered drawer with a visible toggle (also a View-menu item) that closes after navigation. Grid-view file icons were reduced from 40px to 32px **at every width, desktop included** — not gated to phones/tablets. Known gap: the search input in the Drive folder picker runs past the right edge at 390px. Verified live at phone/tablet widths.
...
* **Responsive, added 2026-09-21** On phones the conversation list is the home screen, with a "‹ Chats" back button from a conversation; contact and group details open as overlay panels below 1024px; the per-message menu and reaction strip flip left/right/up to stay inside the window. Verified live at phone/tablet widths.
...
* **Responsive, added 2026-09-21** The groups/labels sidebar becomes a drawer opened from a Menu button in the list header; the chip strip collapses to All / Favourites plus the active label. Verified live at phone/tablet widths.
...
* **Responsive, added 2026-09-21** Below 1024px the three equal columns fold the item type under its name so names are not truncated to a third of the row. Verified live at phone/tablet widths.
...
* **Responsive, added 2026-09-21** Below 1024px the Mailboxes sidebar is an overlay drawer and the list/reader are single-pane; toolbar actions overflow into a `⋯` menu; the toolbar's sidebar toggle opens **and** closes the drawer (it originally only opened). Verified live at phone/tablet widths.
...
* **Responsive, added 2026-09-21** Whiteboard is full-screen on phones with an icon-only header and finger drawing (Pointer Events); Breakout Rooms, Invite and Schedule modals fit 360px; the camera, microphone, meeting-time and category native `<select>`s were replaced by the in-app `SelectMenu`, whose popup cannot open off-screen. The in-call layout, side panels and menus already fit and were left alone. **Verified live with one participant only** at 360/390/768/1440px — a multi-participant grid, screen sharing and real device cameras/microphones were not exercised.
...
* **Responsive, added 2026-09-21** Below 1024px the Activity Bar and inline sidebar are replaced by one drawer (Explorer / Search / Settings, with a close button) opened from a Files button in the tab bar; tree rows grow to 40px, the minimap is off, tab close buttons stay visible on touch, and the Settings page's category list becomes a dropdown on phones. Verified live at phone/tablet widths.
...
* **Responsive, added 2026-09-21** The five view buttons collapse into one dropdown below a very narrow window width (container-measured). Verified live at phone/tablet widths.
...
* **Responsive** Not done — renders at desktop layout on phones and tablets (2026-09-21).
* **Risk** Medium — same expectation gap as Calendar

## Word Book

* **Location** `src/apps/wordbook/` (1,783-line `index.tsx` plus ~40 components/editor modules, ~8,700 lines total)
* **Purpose** A Google-Docs-style word processor: TipTap/ProseMirror editor, real client-side pagination onto discrete A4/Letter/Legal pages, its own native `.book` file format stored through `platform.files` like any other Drive file. **First entry for this app in this document** — it predates this audit round and had accumulated substantial functionality (headings/lists/tables/images/comments-free rich text, export to `.docx`/`.pdf`/`.txt`/`.html`/`.md`, print, sharing) before the session described below, none of which has been independently re-verified here; the bullets below are scoped to what changed in that session.
* **Uses** `platform.files` (`FileService`) for load/save/versions; `MessagingService` for Share to Chat; the shared `platform/menus/` model and `WindowMenu.tsx`'s `MenuPanel`/`placePanel` for every dropdown and context menu in the app, rather than a bespoke popover system.
* **Implemented, session ending 2026-09-17** A large iterative pass driven entirely by user bug reports and feature requests against the existing app, in order:
  * **Ribbon dropdowns** (zoom presets, checklist/bullet/numbered-list style pickers) were fixed after reports that clicking the chevron opened nothing — root cause was `overflow-x: auto` on the ribbon row forcing `overflow-y: auto` too (CSS spec behaviour), clipping the popovers. Fixed by portaling every ribbon popover to `document.body` with `position: fixed`, plus a new shared `clampPopoverPosition()` helper (`ribbon/RibbonPrimitives.tsx`) that flips a popover above/left of its trigger when it would overflow the viewport — reused later by the menu-search results panel for the same reason.
  * **Editing/Viewing mode switcher** moved from a dead ribbon dropdown into a working title-bar control; Viewing hides the ribbon entirely and shows a "read-only" banner rather than leaving disabled-looking buttons visible.
  * **Text/highlight color pickers** rebuilt as the full Google Docs 8×10 palette with a CUSTOM row (`<input type=color>` plus a real `window.EyeDropper` eyedropper where the browser supports it).
  * **Sharing.** The File-menu Share option silently no-opped for a just-created, unsaved document — `ShareModal` bails out on a null `fileItem`, and the file lookup that fed it never fell back for a file the app itself had just created. Fixed with a synthesized fallback `FileItem`. Added a dedicated title-bar Share button and a Share-to-Chat flow (`ShareToChatModal.tsx`) that defaults to the native `.book` format, offers `.docx`/`.pdf`/`.txt`/`.html`/`.md` as alternatives, and requires an explicit confirmation step before sending via `MessagingService.sendFileMessage`.
  * **Font size** gained a Docs-style preset dropdown (`RibbonFontSizeStepper`); **Insert Link** replaced a bare `prompt()` with a real dialog (`LinkModal.tsx`) that can also link to a heading or a bookmark elsewhere in the document. Fixed two bugs found in the process: typing immediately after a link kept extending the link (`Link` mark's `inclusive` was following `autolink`, now explicitly `false`), and links are now plain-click-to-navigate, styled blue/underlined, instead of requiring Ctrl+click with default browser link styling.
  * **Printing** was scoped to just the document (previously `window.print()` printed the entire OS desktop/taskbar/other windows) via a `visibility`-based show-one-subtree CSS trick, then had the browser's own default print header/footer (title, URL, date, page number) suppressed via a zero-margin `@page` rule sized to the document's actual page setup. A **Page Setup dialog** (paper size, orientation, margins) was wired into the Edit menu; a follow-up report of overlapping margin input fields in that dialog was traced to a 2-column CSS grid whose label column was narrower than "Bottom"/"Right" and to grid items' `min-width: auto` default letting that overflow spill past the modal edge — fixed by switching to a single-column layout.
  * **View menu** expanded from two items to a full Docs-style set: a Mode submenu (Editing/Viewing wired to the same switcher as the title bar; Suggesting listed but disabled — no track-changes/proposed-edit storage exists to back it); Comments (listed but disabled — no comment-thread backend exists); an outline-sidebar toggle; the existing Pagination submenu; and four new working toggles — **Show print layout** (swaps the per-page borders/shadow/page-number chrome for one continuous sheet; the underlying content still breaks at the same points, since real continuous reflow would mean changing the pagination engine itself), **Show ruler** (new `PageRuler.tsx`, a read-only inch ruler with shaded margin regions matching the live page setup and zoom — no drag-to-adjust), **Show equation toolbar** (new `EquationToolbar.tsx`, a persistent symbol strip reusing the same palette/insertion pipeline as the existing Insert ▸ Equation dialog), and **Show non-printing characters** (a pilcrow after each paragraph/heading/list item via CSS — not a full formatting-marks engine, so spaces/tabs aren't marked). **Full screen** uses the real browser Fullscreen API and stays in sync if the user exits via Esc.
  * **"Search the menus"** (`MenuSearch.tsx`): a search box that flattens every menu (including submenus) into one searchable list and runs the exact same bound handler the real menu item would; reachable via `Alt+/`. Initially placed in the menu-bar row, then moved into the ribbon toolbar as its last item at the user's request — which required the same portal/viewport-clamp fix described above, since the ribbon row (unlike the menu-bar row) clips ordinary absolutely-positioned children.
  * **Document tabs** (`DocumentTabsSidebar.tsx`, `documentTabs.ts`): the largest change. `.book`'s `BookSection` gained `title`/`emoji`/`parentId`/`order` fields (format v1 → v2, with a migration that turns any existing single-section document into one top-level "Tab 1", so old files keep opening correctly). Each tab is a genuinely independent chunk of document content — nestable into subtabs, addable/deletable/duplicatable/renamable/reorderable/re-parentable, each with an optional emoji chosen from the same character-grid picker Insert ▸ Symbols ▸ Emoji uses. One shared TipTap editor instance is reused across tabs (switching serializes the outgoing tab's `editor.getJSON()` into state and loads the incoming tab's content in) rather than keeping N live editor instances, which keeps the existing pagination/ribbon/plugin stack completely unchanged but means **switching tabs resets ProseMirror undo history** and that **non-`.book` exports (docx/pdf/txt/html/md) and Print still only cover the active tab**, not all tabs merged — only the native `.book` save/Share-to-Chat path serializes every tab. A tab's "Copy link" produces a `#tab:<id>` link that is clickable inside the document (same convention as the app's existing `#bookmark:`/`#heading:` links) and jumps straight to that tab.
* **Responsive, added 2026-09-21** (a later pass than the session above) Below 1024px the ribbon keeps a short set of common controls with a "More options" menu, pages auto-fit-zoom to the available width instead of scrolling sideways, and the title bar drops secondary text so the filename keeps its width. Verified live at phone/tablet widths; this does not extend to the earlier session's tab tree or View-menu toggles, which remain unverified.
* **Status** `UNVERIFIED (UI rendering)` for this session's changes — every change was checked with `tsc --noEmit` (clean throughout) and confirmed to resolve through the Vite dev server (HTTP 200), but **no browser automation was available in that environment**, so nothing above was exercised by an actual click, keystroke, save/reload round trip, or print/export. The user should exercise the tab tree (add/nest/reorder/delete, then save and reload) and the View-menu toggles before relying on them.
* **Known limitations** Suggesting mode and Comments are visible in the Mode/View menus but inert by design (no backend); the ruler doesn't drag; non-printing-characters marks paragraphs only, not spaces/tabs; only the active tab is covered by non-`.book` export formats and by Print.
* **Risk** Medium — substantial new format (v2) and document-model surface (tabs) shipped in one pass with no live-browser or round-trip verification yet; the migration path for pre-existing `.book` files was reasoned through but not run against a real saved file from before this change.
...
* **Responsive, added 2026-09-21** The canvas auto-fits on phones/tablets instead of scrolling; Tools and Properties are overlay drawers with a backdrop; a new in-app colour picker (`components/PaintColorPicker.tsx`) replaces one that opened off-screen. Verified live at phone/tablet widths.
...
* **Responsive, added 2026-09-21** Below 1024px the sidebar is an overlay drawer (closed by default), the toolbar is two rows with a `⋯` "More actions" menu, annotation tools are icon-only on phones, ink uses Pointer Events so a finger draws instead of scrolling, and the zoom control is an in-app `ZoomDropdown` (desktop keeps the native select). Known gap, **pre-existing**: text-layer spans scale incorrectly at every size. Verified live at phone/tablet widths.
...
* **Responsive, added 2026-09-21** Below 1024px the category sidebar is a drawer opened from the header, and the sub-tab pills are measured on phones — those that fit stay inline and the rest move into a `⋯` "More tabs" menu (the active tab always stays visible). Verified live at phone/tablet widths.
...
| --------- | -------- | ------ |
| Window manager | `shell/window-manager/` | `WORKING` — shared chrome, one status bar, portal-rendered menus. Drag and resize keep their state in refs and commit to the store **once on release**; a 120-event drag produces exactly one store write. 44 assertions cover the gesture arithmetic, clamping at every edge and dock size, and the commit count. **The motion itself is unverified** — no browser automation. See [Windows and the dock](../features/shell-windows-and-dock.md). Below 640px every window is forced maximized and between 640 and 1023px windows are clamped to the viewport — see [Phones and tablets](../features/responsive-layout.md). |
| Dock | `shell/taskbar/Dock.tsx`, `dockZone.ts` | `WORKING` — hides whenever a window covers its strip (maximized, dropped over it, or dragged into it mid-gesture), reveals while the cursor is at the bottom edge. 51 assertions cover the full visibility truth table and the live-gesture store. **Not visually reviewed.** Below 1024px the rule changes: it hides while any non-launcher app is open and reappears when that app is minimized or closed (no hover reveal, since there is none by touch). That tier was checked in a browser at phone/tablet widths during the shell pass; the ≥1024px behaviour above was not re-checked. See [Phones and tablets](../features/responsive-layout.md). |

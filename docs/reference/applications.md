...
* **Implemented** Browse, upload, download (single file via a signed URL; multiple items or any folder via a server-streamed zip, `POST /files/download-zip`), rename, move, trash, restore, share, properties, preview, open-with, multi-select, context menus
